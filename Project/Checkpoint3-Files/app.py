    ...
    try:
        ...
        if table_name == "MediaType":
            ...
            platform_id = prompt_with_list(
                "Enter PlatformID (or type 'list' to see available Platforms): ",
                list_platforms,
                conn
            )
            author_name = input("Enter AuthorName: ").strip()
            if not item_name or not release_year or not genre_id or not platform_id or not author_name:
                print("ItemName, ReleaseYear, GenreID, PlatformID, and AuthorName cannot be empty.")
                return
            ...
            cursor.execute("INSERT INTO ItemPlatform (ItemID, PlatformID) VALUES (?, ?)", (item_id, platform_id))
